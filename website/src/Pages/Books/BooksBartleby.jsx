...
export default function BooksBartleby() {
  ...
  const updateImportChapters = useMutation(
    ...
    },
    {
      onSuccess: () => {
        ...
        history.push(
          `/books-bartleby/${params.isbn}/${params?.status}/${chapters?.data[1]?.section_id}`
        );
  ...
  async function importQuestion() {
    if (params?.section_id) {
      ...
      let sectionResponse = await axios.get(
        `https://nk6xemh85d.execute-api.us-east-1.amazonaws.com/prod/solution/${params?.isbn}/section-questions/${params?.section_id}`
      );
  ...
  const UpdateChaptersStatus = useMutation(
    ...
    },
    {
      onSuccess: () => {
        ...
        setUploaded(false);
        history.push(
          `/books-bartleby/${params?.isbn}/import-chapter/${params?.section_id}`
        );
      },
    }
  );
  ...
  async function handleExpertAnswer(id) {
    history.push(
      `/books-bartleby/${params?.isbn}/update-expert-answer/${params?.section_id}/${id}`
    );
  }
  ...
  const MutateExpertAnswer = useMutation(
    ...
    {
      onSuccess: () => {
        queryClient.invalidateQueries([`problems-${params?.section_id}`]);
        queryClient.invalidateQueries([
          `question-answeres-${params?.section_id}-${params?.question_id}`,
        ]);
        setQuestionsData([]);
        setFetching(false);
        setUploaded(false);
        ...
        history.push(
          `/books-bartleby/${params.isbn}/view-uploaded-chapter/${params?.section_id}/${problems[1]?._id}`
        );
      },
    }
  );
          ...
          <div className="main-area-all">
            <div className="dashboard_main-container">
              <div className="dash-main-head">
                <h2>
                  Bartleby Text Books Solution Manuals - ISBN: {params?.isbn}
                </h2>
              </div>
                  ...
                  <div className="p-0 ml-2">
                    <button
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        history.push(
                          `/books-bartleby/${params?.isbn}/import-chapter`
                        );
                  ...
                  <div className="p-0 ml-2">
                    <button
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        history.push(
                          `/books-bartleby/${params?.isbn}/view-uploaded-chapter`
                        );
                    ...
                    <div className="col-md-12 row">
                      <div className="col-md-8 pl-0">
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            if (section_id === "_") {
                              history.push(
                                `/books-bartleby/${params?.isbn}/${params?.status}`
                              );
                            } else {
                              setSectionId(section_id);
                              history.push(
                                `/books-bartleby/${params?.isbn}/${params?.status}/${section_id}`
                              );
                            }
                          }}
                        >
                      ...
                      <div className="col-md-3">
                        ...
                        {questionsData?.length > 0 && (
                          <button
                            className="btn btn-md dark ml-2"
                    ...
                    <div className="col-md-3 pl-0">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          if (e.target.value !== "-")
                            history.push(
                              `/books-bartleby/${params?.isbn}/view-uploaded-chapter/${e.target.value}`
                            );
                        }}
                      >
                            ...
                            problems?.map((problem) => {
                              return (
                                <div
                                  ...
                                  onClick={(e) => {
                                    history.push(
                                      `/books-bartleby/${params?.isbn}/view-uploaded-chapter/${params?.section_id}/${problem?._id}`
                                    );
                                  }}
                                >
                                  {problem?.problem_no}
